...
export default function AITutorChat({ circuitContext, activeTopic = "entanglement" }) {
  ...
  const handleClearCopilotChat = () => {
    if (deleteUserChatHistory) deleteUserChatHistory();
    setMessages([DEFAULT_AURA_MSG]);
  ...
  const handleSendQuery = async (queryText) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          sender: 'aura',
          text: "Operating in offline mode. Entangled Bell states (|Φ+⟩) yield identical outcomes upon measurement with 100% mutual correlation.",
